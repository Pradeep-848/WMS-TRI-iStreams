...
const ShopfloorTracking = () => {
    ...
    const handleScannedValue = (value) => {
        if (activeInput === 'deskArea') {
            setDeskArea(value);
        } else if (activeInput === 'cuttingLineNo') {
            setCuttingLineNo(value);
        }
        setModalVisible(false);
        setActiveInput(null); // reset
    };
    ...
    const handleGetSectionName = () => {
        ...
        if (filteredData.length > 0) {
            ...
            setDeskArea('');
            setSectionName('');
            showSnackbar('Desk Area Not Found', 'error');
        }
    };
    ...
    const handleNavigation = () => {
        if (!deskArea) {
            showSnackbar('Enter Desk Area', 'warning');
            return;
        }
        else if (!cuttingLineNo) {
            showSnackbar('Enter Cutting Line No', 'warning');
            return;
        }
        else {
