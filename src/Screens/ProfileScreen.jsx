...
const ProfileScreen = () => {
  ...
  useEffect(() => {
    const loadRotationPreference = async () => {
      const value = await AsyncStorage.getItem('ROTATION_ANGLE');
      if (value !== null) {
        setRotationAngle(JSON.parse(value));
      } else {
        setRotationAngle(0); // default
        await AsyncStorage.setItem('ROTATION_ANGLE', JSON.stringify(0));
      }
    };
  ...
  // Toggle rotation between 0 and 270
  const toggleRotation = async () => {
    const newValue = rotationAngle === 0 ? 270 : 0;
    setRotationAngle(newValue);
    await AsyncStorage.setItem('ROTATION_ANGLE', JSON.stringify(newValue));
  };
    ...
    <View style={globalStyles.flex_1}>
      <View style={[globalStyles.flex_1, { paddingTop: insets.top }]}>
        ...
        >
          <View style={globalStyles.flex_1}>
            {/* Header */}
            <View style={[globalStyles.px_10, globalStyles.twoInputContainer1]}>
              <Text style={[globalStyles.title1, { color: colors.white }]}>Profile</Text>
              <View>
                <LinearGradient
                  colors={['#6a11cb', '#2575fc']}
                  style={styles.companyContainer}
                >
                  <Icon name="briefcase" size={20} color={colors.white} style={globalStyles.mr_10} />
                  <Text style={[globalStyles.subtitle, globalStyles.txt_center, { color: colors.white }]}>{userData.companyName}</Text>
                </LinearGradient>
              </View>
            </View>

            {/* Profile Info */}
            <View style={[globalStyles.twoInputContainer1, globalStyles.m_10]}>
              <Image style={styles.userImg} source={{ uri: `data:image/jpeg;base64,${userData.userAvatar}` }} />
              <View style={globalStyles.flex_1}>
                <Text style={[globalStyles.title1, { color: colors.white }]}>{userData.userName}</Text>
                <Text style={[globalStyles.body, { color: colors.white }]}>{userData.userEmail}</Text>
              </View>
            </View>
                  ...
                  <View>
                    ...
                    {showCameraSettings && (
                      <View>
                        ...
                        <View style={[globalStyles.twoInputContainer1, globalStyles.my_10]}>
                          <Text style={globalStyles.subtitle_1}>Implement Rotation</Text>
                          <Switch value={rotationAngle === 270} onValueChange={toggleRotation} theme={theme} />
                        </View>
                        ...
                        {loading && (
                          <View style={[globalStyles.my_10, globalStyles.alignItemsCenter]}>
