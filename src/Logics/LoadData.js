...
const LoadData = async (taskKey, clientURL, companyCode, branchCode) => {
    try {
        ...
        if (taskKey === 'fetchMaterialsList') {
            const MaterialList_Params = {
                CompanyCode: companyCode,
                BranchCode: branchCode
            };
            const MaterialsList = await callSoapService(clientURL, 'Invt_Get_AllMaterialsList_Mobile', MaterialList_Params);

            await AsyncStorage.setItem('MaterialsList', JSON.stringify(MaterialsList));
        }

        if (taskKey === 'fetchStoreLocationList') {
            const StoreLocationList = await callSoapService(clientURL, 'getAll_Store_Locations', '');

            await AsyncStorage.setItem('StoreLocationList', JSON.stringify(StoreLocationList));
        }
    } catch (error) {
