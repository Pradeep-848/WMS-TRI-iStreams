...
export const callSoapService = async (url, methodName, parameterDetails) => {
  try {
    const raw = await AsyncStorage.getItem("doConnectionPayload");
    const storedParameter = raw ? JSON.parse(raw) : null;

    if (storedParameter && typeof storedParameter === 'object') {
      await callSoapServiceforMethods(url, "doConnection", storedParameter);
    } else {
